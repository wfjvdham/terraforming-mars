...
export class Game implements IGame, Logger {
  ...
  private setFirstPlayer(first: IPlayer) {
    if (this.playersInGenerationOrder && this.playersInGenerationOrder.length > 0) {
      this.first = this.playersInGenerationOrder[0];
    } else {
      this.first = first;
  ...
  public playerIsFinishedWithResearchPhase(player: IPlayer): void {
    this.deferredActions.runAllFor(player, () => {
      ...
      if (this.researchedPlayers.size === this.players.length) {
        ...
        // Set next generation order based on passing order
        if (this.passedPlayers.length === this.players.length) {
          const newOrder = this.passedPlayers.map((id) => this.getPlayerById(id));
          this.playersInGenerationOrder = newOrder;
